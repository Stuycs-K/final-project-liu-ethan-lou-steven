class Button {
  ...
  public void display(boolean pick, PImage img, PFont font, float txtSize) {
    textFont(font);
    if (pick) {
      float shrink=1;
      ...
      if (!label.equals("Home")) {
        text(getLabel(), (2*x+w-10)/2-3*getLabel().length()*txtSize/11-5, (2*y-h)/2+5);
      }
    }
    else {
      ...
      textSize(txtSize);
      if (!label.equals("Home")) {
        text(getLabel(), (2*getX()+getWidth())/2-3*getLabel().length()*txtSize/11-10, (2*getY()-getHeight())/2+5);
      }
    }
